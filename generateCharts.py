...
def generateCharts(api_url, proxy=()):
    ...
    #for each month and year accumulate the volume
    #groupedPerMonth = df.groupby([pd.Grouper(key='timestamp', freq='M')]).agg({'premium': 'count', 'volume': 'sum'})
...
def generateLineplot(groupedPerDay, title, xlabel, ylabel, yfield, aspect, filename):
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
